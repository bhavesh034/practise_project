...
class SubcategoryController extends Controller
{
    ...
    public function index()
    {
        ...
        return view('Backend.Blog.Subcategory.subcategory')->with('category_id', $categories);
    }
    ...
    public function save(Request $request)
    {
        ...
        if ($id) {
            $save = DB::table('blog_subcategory')->where('id', $id)->update($datainsert);
            $data['status'] = 1;
            $data['massage'] = "Update Record successfully";
        } else {
            $save = DB::table('blog_subcategory')->insert($datainsert);
            $data['status'] = 1;
    ...
    public function list()
    {
        ...
        $data = DB::table('blog_subcategory')
            ->join('blog_category', 'blog_category.id', 'blog_subcategory.category_id')
            ->select('blog_subcategory.*', 'blog_category.category_name')
            ->get();
    ...
    public function get_detail(Request $request)
    {
        $id = $request->id;
        if ($id) {
            $data = DB::table('blog_subcategory')->where('id', $id)->first();
            // echo '<pre>';
            // print_r($data);
            // die;
            return json_encode($data);
